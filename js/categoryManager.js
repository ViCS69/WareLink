...
import { collection, addDoc, query, where, getDoc, getDocs, deleteDoc, doc, updateDoc } from "https://www.gstatic.com/firebasejs/10.1.0/firebase-firestore.js";
...
async function deleteCategory(categoryName, storeId) {
    try {
        if (!confirm(`Сигурни ли сте, че искате да изтриете категория "${categoryName}"?`)) {
            return;
        }
...
async function loadCategories(storeId = null) {
    ...
    try {
        ...
        if (categoriesList) {
            ...
            allCategoriesBtn.classList.add("flex", "items-center", "justify-center", "px-4", "py-2", "bg-gray-800", "text-white", "w-full", "hover:bg-gray-700", "mb-2");
        ...
        categorySnapshot.forEach((doc) => {
            ...
            if (categoriesList) {
                ...
                categoryBtn.classList.add("flex", "items-center", "justify-between", "px-4", "py-2", "hover:bg-gray-700", "text-white", "w-full", "group");
                ...
                if (isStorePage) {
                    ...
                    deleteBtn.innerHTML = "×";
                    deleteBtn.classList.add("text-gray-400", "hover:text-red-500", "text-xl", "font-bold", "px-2", "leading-none", "transition-colors", "duration-200");
...
export { addCategory, loadCategories, checkIfCategoryExists, deleteCategory, changeProductCategory, getCategoriesForStore, populateCategoryDropdown, getStoreId };